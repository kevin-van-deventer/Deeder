...
const MapsPage = () => {
  ...
  const fetchDeeds = async () => {
    try {
      ...
      setUnfulfilledDeeds(
        response.data.filter((deed) => deed.status === "unfulfilled").length
      )
    } catch (error) {
  ...
  const handleVolunteer = async (deedId, requesterId) => {
    ...
    try {
      const response = await axios.post(
        `${process.env.REACT_APP_API_BASE_URL}/deeds/${deedId}/volunteer`,
        {},
            ...
            >
              {deeds.map((deed) => (
                <Marker
                  ...
                  icon={
                    deed.deed_type === "one-time" ? oneTimeIcon : materialIcon
                  } // Set marker icon based on deed type
                />
              ))}
              ...
              {selectedDeed && (
                ...
                >
                  <div>
                    <p>
                      <strong>{selectedDeed.description}</strong>
                    </p>
                    <p>
                      <strong>{selectedDeed.deed_type}</strong>
                    </p>
                    <p>
                      <strong>Deeders:</strong> {selectedDeed.volunteer_count}
                    </p>
          ...
          {deeds.length === 0 ? (
            ...
            deeds.map((deed) => (
              <div key={deed.id} className="deed-card">
                <p>
                  <strong>{deed.description}</strong>
                </p>
                <p>{deed.address}</p>
                <p>
                  <strong>Type:</strong> {deed.deed_type}
                </p>
                <button
                  id="deedButton"
